...
export function EditKeHoachSheet({ open, onOpenChange, userId = "system", plan }: EditKeHoachSheetProps) {
  ...
  const form = useForm<UpdateKeHoachCSKHFormData>({
    resolver: zodResolver(updateKeHoachCSKHSchema),
    defaultValues: {
      ID_KH: "",
      ID_LH: "",
      ID_LCS: "",
  ...
  useEffect(() => {
    if (open && plan) {
      form.reset({
        ...
        ID_DD: plan.ID_DD || "",
        ID_LCS: plan.ID_LCS || "",
        HINH_THUC: plan.HINH_THUC as any,
        DIA_DIEM: plan.DIA_DIEM || "",
        NOI_DUNG_TD: plan.NOI_DUNG_TD || "",
        GHI_CHU_NHU_CAU: plan.GHI_CHU_NHU_CAU || "",
        TRANG_THAI: plan.TRANG_THAI as any,
        TG_TU: new Date(plan.TG_TU),
        TG_DEN: new Date(plan.TG_DEN),
        ID_KQ: plan.ID_KQ || "",
  ...
  const onSubmit = (data: UpdateKeHoachCSKHFormData) => {
    if (!plan) return;
    startTransition(async () => {
      ...
      const result = await updateKeHoachCSKH(plan.ID_CSKH, submitData, userId);
      if (!result.success) {
        toast.error(typeof result.error === 'string' ? result.error : "Lỗi cập nhật");
        return;
      }
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="mb-4">
                  ...
                  <Select onValueChange={field.onChange} value={field.value}>
                    <FormControl>
                      <SelectTrigger className={cn("font-medium", field.value === 'HOAN_THANH' ? 'text-green-700 bg-green-50 border-green-200' : field.value === 'HUY' ? 'text-red-700 bg-red-50 border-red-200' : 'text-yellow-700 bg-yellow-50 border-yellow-200')}>
                        <SelectValue />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
                      <SelectItem value="CHO_BAO_CAO" className="text-yellow-700 font-medium">Chờ báo cáo</SelectItem>
                      <SelectItem value="HOAN_THANH" className="text-green-700 font-medium">Hoàn thành</SelectItem>
                      <SelectItem value="HUY" className="text-red-700 font-medium">Hủy</SelectItem>
                    </SelectContent>
                  </Select>
                  <FormMessage />
                </FormItem>
